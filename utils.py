...
async def process_image_async(image_path: str, alphabet: str) -> tuple:
    ...
    prompt = (
        ...
        f"3. Identify the bounding boxes of ONLY the non-text pure graphical elements (e.g. outline of engines/devices). "
        f"CRITICAL LAYOUT RULES for the 'text' field:\n"
        f" - A continuous paragraph MUST be on a SINGLE line in your output. Do not break sentences mid-paragraph. Use \\n ONLY for actual new paragraphs.\n"
        f" - If a paragraph or line is horizontally centered (like a title), start the line exactly with the tag [CENTER].\n"
        f" - If a paragraph has a first-line indentation, start it exactly with the tag [INDENT].\n"
        f" - The graphical diagram's bounding box `ymin` MUST be strictly greater than `text_bottom_y`.\n"
        f"Return ONLY valid JSON exactly like this: "
        f"{{\n"
        f"  \"text\": \"[CENTER]Document Title\\n\\n[INDENT]First paragraph text...\",\n"
        f"  \"text_bottom_y\": 250,\n"
