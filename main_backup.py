...
def get_main_menu_keyboard(is_admin=False):
    keyboard = [
        [InlineKeyboardButton(text="📸 Rasmni hujjatga aylantir", callback_data="mode_image")],
        ...
        [InlineKeyboardButton(text="🎤 Ovozni hujjatga aylantir", callback_data="mode_voice")],
        [InlineKeyboardButton(text="🌐 Rasmni tarjima qilib hujjat", callback_data="mode_translate")],
        [InlineKeyboardButton(text="🔄 Lotin ↔ Кирилл o'girish", callback_data="mode_convert")]
    ]
    if is_admin:
        keyboard.append([InlineKeyboardButton(text="📊 Hisobim", callback_data="myaccount"),
                         InlineKeyboardButton(text="🏆 Reyting", callback_data="leaderboard")])
    else:
        keyboard.append([InlineKeyboardButton(text="📊 Hisobim", callback_data="myaccount")])
        
    keyboard.append([InlineKeyboardButton(text="❓ Qanday ishlaydi?", callback_data="help_menu")])
...
def get_feedback_keyboard(doc_hash: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="👍 Yaxshi", callback_data=f"fb_good_{doc_hash}"),
            InlineKeyboardButton(text="👎 Yomon", callback_data=f"fb_bad_{doc_hash}"),
        ],
...
async def translate_alphabet_callback(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer("Sifat qanday?", reply_markup=get_feedback_keyboard(doc_hash))
...
async def smart_clipboard_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("Sifat qanday?", reply_markup=get_feedback_keyboard(doc_hash))
